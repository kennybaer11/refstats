...
// Get filters from query string (as strings!)
...
// Aggregation level: overall, competition, season, phase, phasedetail
...
// Prepare SELECT parts and GROUP BY depending on grouping depth
$select_parts = [];
$groupBy = ["r.referee_id","r.referee_name"];

// Competition is always included except in "overall"
if ($group !== 'overall') {
...
// Season included for season, phase, phasedetail
if ($group === 'season' || $group === 'phase' || $group === 'phasedetail') {
...
// Phase included for phase, phasedetail
if ($group === 'phase' || $group === 'phasedetail') {
...
// Phase Detail only in phasedetail
...
$sql = "
    SELECT 
        ...
        SUM(ms.total_2min) AS total_2min,
        SUM(ms.total_5min) AS total_5min,
    ...
    FROM refereessql r
    JOIN matchesmysql m 
      ON (r.referee_id = m.ref1_id OR r.referee_id = m.ref2_id)
    JOIN matchesstatssql ms 
...
// Append grouping columns only if requested
...
// Build filter option lists (unchanged)
$filters = [
    "competitions" => [],
    "seasons" => [],
    "phases" => [],
