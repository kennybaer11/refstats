...
// Get pair_id from query string
$pairId = $_GET['pair_id'] ?? null;
if (!$pairId) {
    die(json_encode(["error" => "Missing pair_id"]));
}

// fetch the pair's name
$pairStmt = $pdo->prepare("SELECT pair_name FROM refereepairsql WHERE pair_id = ?");
$pairStmt->execute([$pairId]);
$pairData = $pairStmt->fetch();
$pairName = $pairData['pair_name'] ?? null;

if (!$pairData) {
    die(json_encode(["error" => "Pair not found"]));
}
