...
if (!$referee_id) {
    ...
    echo json_encode(["error" => "Missing referee_id"]);
    exit;
}

// ---- Get referee name ----
$sqlRef = "SELECT referee_id, referee_name FROM refereessql WHERE referee_id = :ref";
$stmt = $pdo->prepare($sqlRef);
$stmt->execute(['ref' => $referee_id]);
$refereeInfo = $stmt->fetch();
if (!$refereeInfo) {
...
$sqlPhases = "
    ...
    WHERE m.ref1_id = :ref1 OR m.ref2_id = :ref2
...
$stmt->execute(['ref1' => $referee_id, 'ref2' => $referee_id]);
...
// Add top-level referee info
$output['referee_id'] = $refereeInfo['referee_id'];
$output['referee_name'] = $refereeInfo['referee_name'];
...
unset($comp, $season); // break references
